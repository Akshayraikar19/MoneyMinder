...
const UnverifiedOfficers = () => {
    ...
    const sanctionsChartData = {
        labels: sanctions.map(sanction => `Year ${sanction._id.toString()}`), // Convert _id to string
        datasets: [
            {
                label: 'Loan Sanctions',
                data: sanctions.map(sanction => sanction.count),
                backgroundColor: sanctions.map(() => 
        ...
        <div className="container" style={{ backgroundColor: "#f0f0f0", padding: "20px" }}>
            ...
            <div className="mb-4">
                ...
                <div style={{ maxWidth: '600px', margin: 'auto' }}>
                    <Pie data={sanctionsChartData} options={{ responsive: true, maintainAspectRatio: false }} />
                </div>
            </div>

            <div>
                <h2 className="text-center">Retention Rate</h2>
                <div style={{ maxWidth: '600px', margin: 'auto' }}>
